...
# app.R - SEIR with direct split to ICU (20% of hospitalisations) and ICU stay = 10 days
library(shiny)
library(deSolve)
library(tidyverse)
library(openxlsx)
...
server <- function(input, output) {
  ...
  sim_data <- eventReactive(input$run, {
    ...
    p_die_hosp_vec <- c(young = 0.02, adult = 0.25, elderly = 0.5) # fraction of hospitalized who die (overall)
    sigma <- 1/2
    gamma <- 1/5
    ...
    seird_model_age <- function(time, state, parameters) {
      with(as.list(state), {
        ...
        for (i in 1:3) {
          ...
          # Outflow rates (per day) from compartments (simple exponential durations)
          # Non-ICU hospital: total outflow rate = 1/Ht
          # ICU: total outflow rate = 1/Ct
          out_h_rate <- 1 / Ht
          out_c_rate <- 1 / Ct
          
          # Split outflows into recover / death using p_die_hosp attribution:
          # assume fraction p_die_hosp of ALL hospitalized (non-ICU + ICU) die.
          # allocate deaths proportionally to where people are (by the inflow fraction).
          # Thus deaths from non-ICU = p_die_hosp * (1 - p_icu) / Ht * H
          # and deaths from ICU = p_die_hosp * p_icu / Ct * C
          # mu_h <- (p_die_hosp * (1 - p_icu)) / Ht
          # rho_h <- ((1 - p_die_hosp) * (1 - p_icu)) / Ht
          # 
          mu_h <- p_die_hosp / Ht
          rho_h <- (1 - p_die_hosp) / Ht
          
          mu_c <- (p_die_hosp * p_icu) / Ct
          rho_c <- ((1 - p_die_hosp) * p_icu) / Ct
          ...
          # Hospital (non-ICU) and ICU dynamics with direct inflow split
          # dH <- inflow_h_nonicu - out_h_rate * H  # outflow broken into mu_h and rho_h later for D/R totals
          dH <- p_hosp * gamma * I - (rho_h + mu_h) * H
          dC <- p_icu * p_hosp * gamma * I - out_c_rate * C
