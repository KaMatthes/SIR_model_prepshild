...
server <- function(input, output) {
  ...
  sim_data <- eventReactive(input$run, {
    ...
    get_vaccination_rate <- function(time) {
      if (time >= input$pervac1 & time < input$pervac2) c(young=v1[1], adult=v1[2], elderly=v1[3])
      else if (time >= input$pervac2 & time < input$pervac3) c(young=v2[1], adult=v2[2], elderly=v2[3])
      else if (time >= input$pervac3) c(young=v3[1], adult=v3[2], elderly=v3[3])
