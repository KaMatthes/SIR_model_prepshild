...
get_intervention_factor <- function(time) {
  ...
  else return(1 - 0.3* (1 - exp(-0.1 * (time - 100))))
}
...
df1 <- data.frame(time= 50,
                  prop = c(500000,25000, 40000), 
                  fac=c("Susceptible", "Exposed","Infected"),
                  text = "no NPI") %>%
  mutate(
    fac  = factor(fac , levels=c("Susceptible","Exposed","Infected"))
...
ggplot(dt, aes(x = time)) +
  facet_wrap(~fac, ncol=3, scales = "free_y") +
  annotate("rect",xmin=0,xmax=100,ymin=-Inf,ymax=Inf,alpha=0.2,fill="grey90") +
  geom_text(data = df1,
            aes(x=time, y=prop,label = text),
            angle = 90,
            size=ann_text_size) +
  annotate("rect",xmin=100,xmax=Inf,ymin=-Inf,ymax=Inf,alpha=0.2,fill="grey40") +
  geom_text(data = df2,
            aes(x=time, y=prop,label = text),
            angle = 90,
            size=ann_text_size) +
  geom_line(aes(y = prop, color = fac), lwd=lwd_size) +
  labs(title = "Dengue - Severe Scenario - 100 days",
       x = "Days",
       y = "Mosquitoes")+
  scale_color_manual("",
                     # breaks=c("young","adult","elderly"),
...
ggsave("figures/dengue/severe_age_meas100_M.png",h=8,w=20)
