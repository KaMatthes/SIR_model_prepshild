...
get_intervention_factor <- function(time) {
  if (time < 80) return(1.0)
  else return(1 - 0.3* (1 - exp(-0.1 * (time - 80))))
}
...
dt_h <- out_df[, c(1,grep("_m", names(out_df)))] 

dt_hd <- dt_h[, c(1,grep("S_", names(dt_h)))] %>%
  mutate(fac="S") %>%
  rename(prop = S_m)
...
dt_hh <- dt_h[, c(1,grep("E_", names(dt_h)))] %>%
  mutate(fac="E")%>%
  rename(prop = E_m)
...
dt <- rbind(dt_hd, dt_hh, dt_hi) %>%
  mutate(
    fac = case_when(fac == "S" ~ "Susceptible",
                           fac == "E" ~ "Exposed",
                           fac == "I" ~ "Infected"),
    fac  = factor(fac , levels=c("Susceptible", "Exposed","Infected"))
  )

# df1 <- data.frame(time= 10,
#                   prop = c(300000,1.2, 0.7), 
#                   fac=c("Susceptible", "Exposed","Infected"),
#                   text = "no NPI") %>%
#   mutate(
#     fac  = factor(fac , levels=c("Susceptible", "Exposed","Infected"))
#   )
...
#             angle = 90,
#             size=ann_text_size) +
#   geom_line(aes(y = prop, color = fac), lwd=lwd_size) +
...
# 
# ggsave("figures/dengue/mild_age_meas40_M.png",h=10,w=20)


df1 <- data.frame(time= 40,
                  prop = c(500000,6000, 9000), 
                  fac=c("Susceptible", "Exposed","Infected"),
                  text = "no NPI") %>%
  mutate(
    fac  = factor(fac , levels=c("Susceptible","Exposed","Infected"))
...
ggplot(dt, aes(x = time)) +
  facet_wrap(~fac, ncol=3, scales = "free_y") +
  annotate("rect",xmin=0,xmax=80,ymin=-Inf,ymax=Inf,alpha=0.2,fill="grey90") +
  geom_text(data = df1,
            aes(x=time, y=prop,label = text),
            angle = 90,
            size=ann_text_size) +
  annotate("rect",xmin=80,xmax=Inf,ymin=-Inf,ymax=Inf,alpha=0.2,fill="grey40") +
  geom_text(data = df2,
            aes(x=time, y=prop,label = text),
            angle = 90,
            size=ann_text_size) +
  geom_line(aes(y = prop, color = fac), lwd=lwd_size) +
  labs(title = "Dengue - Severe Scenario - 80 days",
       x = "Days",
       y = "Mosquitoes")+
  scale_color_manual("",
                     # breaks=c("young","adult","elderly"),
...
ggsave("figures/dengue/severe_age_meas80_M.png",h=8,w=20)
